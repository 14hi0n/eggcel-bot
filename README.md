...
| `MEME_EVENT_PROBABILITY`  | `0.1`                                     | С указаной вероятностью читает файл `events.txt` и добавляет случайную строку из него в основной промпт |
...
Так же можно создать файл `events.txt` и задать ему шанс через переменную `MEME_EVENT_PROBABILITY`. При каждой генерации с вероятностью `MEME_EVENT_PROBABILITY` в промпт добавляется одна случайная строка из него. Это позволяет с низким шансом сгенерировать что-то странное вне шаблонов.  
